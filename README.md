...
5. **Monthly statements.** Per-vendor, per-month snapshot computed strictly from the points log: opening balance, accepted, earn/redeem liability, top-ups, withdrawals, expired, closing, required reload, surplus. Simplified status flow: **Draft → Finalized & Sent** (one-click "Finalize & Send Email" combines locking the numbers with sending the PDF). Admin can revert to Draft if a fix is needed.
6. **PDF + email + vendor portal.** Statements render to PDF via dompdf, emailed to vendors (WYSIWYG-editable template with tokens), and downloadable by vendors from a portal shortcode.
