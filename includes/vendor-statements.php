...
function nc_statement_build_pdf_html( $row ) {
    ...
    // Branding (scope item 8): header text, logo, background colour/image.
    $brand         = function_exists( 'nc_statement_branding' )
        ? nc_statement_branding()
        : array( 'header' => $site . ' — Monthly Statement', 'logo_id' => 0, 'bg_color' => '', 'bg_image_id' => 0 );
    $brand_header  = $brand['header'];
    $brand_logo    = function_exists( 'nc_statement_branding_data_uri' ) ? nc_statement_branding_data_uri( $brand['logo_id'] ) : '';
    $brand_bg_img  = function_exists( 'nc_statement_branding_data_uri' ) ? nc_statement_branding_data_uri( $brand['bg_image_id'] ) : '';
    $brand_bg_col  = $brand['bg_color'];
    $brand_note    = isset( $brand['note'] ) ? $brand['note'] : '';
    $brand_footer  = isset( $brand['footer'] ) ? $brand['footer'] : 'For disputes / clarification, please contact Nation Club admin.';
    ...
    <head>
        ...
        <style>
            ...
            .brand-head { text-align: center; margin-bottom: 6px; }
            .brand-logo { max-height: 70px; margin: 0 auto 8px; display: inline-block; }
            <?php if ( $brand_bg_col || $brand_bg_img ) : ?>@page { margin: 0; }
            html { margin: 0; padding: 0; }
            body { margin: 0; padding: 0; min-height: 100%; }
            .wrap { padding: 30px 34px; }<?php endif; ?>
            <?php if ( $brand_bg_col ) : ?>body { background-color: <?php echo esc_html( $brand_bg_col ); ?>; }<?php endif; ?>
            <?php if ( $brand_bg_img ) : ?>body { background-image: url('<?php echo $brand_bg_img; ?>'); background-repeat: no-repeat; background-position: center center; background-size: cover; }<?php endif; ?>
        </style>
    </head>
    <body>
    <div class="wrap">
        <div class="brand-head">
            <?php if ( $brand_logo ) : ?><img class="brand-logo" src="<?php echo $brand_logo; ?>" alt=""><?php endif; ?>
            <h1><?php echo esc_html( $brand_header ); ?></h1>
        </div>
        <table class="meta">
            <tr>
        ...
        <?php if ( trim( $brand_note ) !== '' ) : ?>
        <p style="margin-top:18px;padding:9px 11px;background:#f7f2f4;border-left:3px solid #8b1c3b;color:#555;font-size:10px;line-height:1.5">
            <?php echo nl2br( esc_html( $brand_note ) ); ?>
        </p>
        <?php endif; ?>

        <div class="footer">
            Generated by <?php echo esc_html( $site ); ?> on <?php echo esc_html( mysql2date( 'M j, Y H:i', $row->generated_at ) ); ?>.<br>
            <?php echo nl2br( esc_html( $brand_footer ) ); ?>
        </div>
    </div>
    </body>
    </html>
